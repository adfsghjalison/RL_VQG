...
if len(sys.argv)<5:
	print 'python final_train.py [input] [data_num] [plan] [EPOCH] ([load_weights]) '
	sys.exit()
...
print("\ntraining ...")
vqa_model.fit([F, C, Q], Y, batch_size=BATCH_SIZE, nb_epoch=EPOCH, validation_split=0.05)
vqa_model.save_weights('model/final/'+plan+'/'+(sys.argv[4] if len(sys.argv)==5 else str(int(sys.argv[5])+EPOCH))+'.h5')
