...
def get_VQA_model(ans_size):
	...
	vqa_model = VQA_MODEL(output_num = ans_size, path = 'model/final/'+plan+'/60.h5')
...
vqa_model.fit([X1, X2], Y, batch_size=BATCH_SIZE, nb_epoch=30, validation_split=0.05)
vqa_model.save_weights('model/final/'+plan+'/130.h5')

vqa_model.fit([X1, X2], Y, batch_size=BATCH_SIZE, nb_epoch=30, validation_split=0.05)
vqa_model.save_weights('model/final/'+plan+'/160.h5')
