...
model = type_model(path='model/'+modeltype+'/500.h5', type_size=type_size)
...
for wei in [200, 250, 300]:
    output = open('output/qg/sample-'+str(wei), 'w')
    model = qg_model(path='model/'+modeltype2+'/'+str(wei)+'.h5', vocab_size=vocab_size)
